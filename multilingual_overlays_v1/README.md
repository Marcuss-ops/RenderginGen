...
|---|---|---|
| Phrase (text) | `phrase_01_lower_third_safe` … `phrase_05_slide_left_punch` | single official text preset `phrase_default` + 5 distinct official motions (`liquid_glass_ripple`, `kinetic_stamp_impact`, `editorial_push_in`, `neon_flicker_ignite`, `hologram_scanline_build`) |
...
`phrase_default` — each with a distinct official motion so the five overlays stay
...
Checked-in manifests, job snapshots, plans, and render reports are immutable
run evidence, not manifests to resubmit after a compiler/catalog migration. New
batches generated by the commands below use the current `phrase_default` style
and catalog-selected `motion_id`; regenerate into a separate output path when
re-running so the historical artifacts continue to describe the renders they
actually produced.
