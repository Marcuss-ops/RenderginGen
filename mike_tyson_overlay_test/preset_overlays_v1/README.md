...
The historical run rendered 10 text overlays with the retired `apple_v2`
visual preset and distinct official `motion_id` values, plus 5 entity image
overlays with the official image presets. Current manifests are rebuilt with the
single `phrase_default` text style and keep animation independently selected by
`motion_id`. Each image preset enters over **37 frames = 1.5 s at 24 fps**
(focus and scale reveal a larger scale change, slide presets travel 360 px, all
four combine with a fade). Each clip is 5 seconds at 1920×1080, 24 fps, on the
...
- The historical checked-in `manifest.json`, `plans/`, and `docs/` are retained
  as evidence for the legacy run and may reference retired IDs. Rebuild output
  goes to `/tmp` (as shown below) so historical evidence is never mistaken for
  a current, renderable manifest.
...
../../renderinggen/bin/batch-build \
  ...
  -out /tmp/tyson-overlays-manifest.json -plans-dir /tmp/tyson-overlays-plans
...
../../renderinggen/bin/batch-run -manifest /tmp/tyson-overlays-manifest.json \
  -report /tmp/tyson-overlays-timing.json -snapshot /tmp/tyson-overlays-jobs.json \
  -download-dir /tmp/tyson-overlays-renders -serve-assets ../.. -serve-addr 127.0.0.1:8099

# 3. certify the artifacts: structure + content address + glyph ink.
../../renderinggen/bin/batch-verify -manifest /tmp/tyson-overlays-manifest.json \
  -report /tmp/tyson-overlays-verify.json -stage /tmp/tyson-verify
```
