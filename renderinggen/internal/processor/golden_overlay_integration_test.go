...
func TestGoldenSemanticOverlayJobV1Compiles(t *testing.T) {
	...
	for _, layer := range concrete.Layers {
		if layer.ID == "" || layer.Type == "" {
			t.Fatalf("compiled layer has no id/type: %+v", layer)
		}
		layerIDs[layer.ID] = true
	}
	for _, want := range []string{"background:image", "important_phrase", "important_word"} {
		if !layerIDs[want] {
			t.Fatalf("compiled layers missing %q (have %v)", want, layerIDs)
		}
	}
}
...
//	+ "QUESTO CAMBIA TUTTO" (IMPORTANT_PHRASE, phrase_default, f20-60)
//	+ "APPLE"               (IMPORTANT_WORD,   phrase_default, f65-95)
//
...
func TestProcessGoldenSemanticOverlayJobV1(t *testing.T) {
	...
	// The golden plan carries phrase + word + the background image overlay,
	// no entities: the counters must reflect PipelineGen's plan.
	if rec.ImportantPhraseCnt != 1 || rec.ImportantWordCnt != 1 || rec.ImageCount != 1 || rec.EntityCount != 0 {
