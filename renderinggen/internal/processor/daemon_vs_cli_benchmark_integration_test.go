...
// the SAME GoldenOverlayJobV2 workload:
//
...
func TestDaemonVsCLIBenchmarkGoldenOverlay(t *testing.T) {
	...
	cli := &chronon.Client{Home: home, Backend: backend}
	...
	for _, phase := range []string{"cold", "warm"} {
		...
		if err := cli.Render(ctx, chronon.RenderRequest{
			...
			Requirements: chronon.ExecutionRequirements{
				Backend: backend, CompositionRequired: true,
				CPUFallbackAllowed: true, PacketCopyAllowed: true,
			},
		}); err != nil {
	...
	fmt.Printf("\n=== RenderingGen daemon vs CLI benchmark (GoldenOverlayJobV2) ===\n")
