// conformance_rules_test.go covers the RULES themselves: that each forbidden
// marker is actually detected in a carrier that escapes its exclusion list, that
// the exclusion lists are live rather than decorative, and that the scanned
// surface (extensions, Dockerfiles, package dirs) is what it claims to be.
//
// Split out of conformance_test.go, which had grown to 911 lines covering four
// unrelated concerns — rule behaviour, the ratchet baseline, the documentation,
// and the CI workflows. A failure in any of them printed the same file name, so
// the first thing a reader had to do was work out which half they were in.
